...
app.directive('xgTable',function($filter){
    return{
        ...
        '<span>条</span>'+
        '</div>'+
        '<input type="text" ng-show="searching" class="form-control rounded" ng-model="searchText" style="width: 200px;display: inline-block;float: right;margin-top:7px" placeholder="搜索">'+
        '<ng-transclude></ng-transclude>'+
        '<div ng-show="sliceData.length>0">'+
        ...
        '</div>'+
        '<p ng-show="sliceData.length==0" style="height:50px;line-height:50px;margin-left: 10px;">没有数据</p>'+
